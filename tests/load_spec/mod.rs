...
fn load_template_a() {
    ...
    let a_spec = TemplateSpec::from_dir("./test_resources/template_a", settings).unwrap();

    // 1. test base_path
    assert_eq!(a_spec.base_path().unwrap(), Path::new("./test_resources/template_a"));

    // 2. test embeddings
    let embeddings = a_spec.embeddings();
    assert_eq!(embeddings.len(), 1);
    let emb_0 = embeddings.get("portfolio").unwrap();
    assert_eq!(emb_0.path, Path::new("./test_resources/template_a/portfolio.pdf"));
    ...
    assert_eq!(text.path(), Path::new("./test_resources/template_a/text/mail.txt"));
    ...
    assert_eq!(html.path(), Path::new("./test_resources/template_a/html/mail.html"));
    ...
    assert_eq!(logo.path, Path::new("./test_resources/template_a/html/logo.png"));
